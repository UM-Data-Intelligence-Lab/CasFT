...
import model.diffeq_layers as diffeq_layers
...
class IntensityODEFunc(nn.Module):
    ...
    def get_intensity(self, tpp_state):
        return F.softplus(self.intensity_fn(tpp_state[..., :self.hdim]))
...
class NeuralPointProcess(TemporalPointProcess):
    ...
    def __init__(self, cond_dim=0, hidden_dims=[64, 64, 64], cond=False, style="split", actfn="softplus", hdim=None, separate=1, tol=1e-6, otreg_strength=0.1, solver="dopri5"):
        super().__init__()
        if not cond:        #默认为True
            cond_dim = 0
        self.cond = cond        #是否加入空间带来的影响
        self.cond_dim = cond_dim        #表示的是空间的dim
        self.hdim = hidden_dims[0] if hdim is None else hdim
        self.solver = solver
        assert self.hdim % 2 == 0
        self._init_state = nn.Parameter(torch.randn(hidden_dims[0]) / math.sqrt(hidden_dims[0]))    #初始化的参数（32，）
        ...
        self.ode_solver = TimeVariableODE(intensity_odefunc, atol=tol, rtol=tol, energy_regularization=otreg_strength, method=solver)

    def logprob(self, event_times, spatial_locations, input_mask, t0, t1):
        intensities, Lamdas, prejump_hidden_states, _ = self.integrate_lambda(event_times, spatial_locations, input_mask, t0, t1, self.solver)
        return intensities, Lamdas, prejump_hidden_states

    def get_intensity(self, state):
        return self.ode_solver.func.get_intensity(state)

    def integrate_lambda(self, event_times, spatial_location, input_mask, t0, t1, method, nlinspace=1):
        """
        Args:
            event_times: (N, T)
        ...
        for i in range(T):
            ...
            state_traj = self.ode_solver.integrate(t0, t1_i, state, nlinspace=nlinspace, method=method)
        ...
        for i in range(len(t1)):
            t1_i = [t1[i]]
            t1_i = t1_i if torch.is_tensor(t1_i) else torch.tensor(t1_i)
            t1_i = t1_i.expand(N).to(event_times)
            state_traj = self.ode_solver.integrate(t0, t1_i, state, nlinspace=nlinspace, method=method)

            hiddens = state_traj[1][1:]
            Lambda = state_traj[0][1]
            Lamdas.append(Lambda)
            prejump_hidden_states.append(hiddens)
            intensities.append(self.get_intensity(hiddens).reshape(-1))
        ...
        return intensities, Lamdas, prejump_hidden_states, None
