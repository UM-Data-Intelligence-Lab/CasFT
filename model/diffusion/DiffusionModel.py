...
class GaussianDiffusion_ST(nn.Module):
    ...
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False,cond=None):
        ...
        return ModelPrediction(pred_noise, x_start), None
    ...
    def ddim_sample(self, shape, clip_denoised = True, cond=None):
        ...
        for time, time_next in time_pairs:
            ...
            self_cond = x_start if self.self_condition else None
            pred, *_ = self.model_predictions(img, time_cond, self_cond, clip_x_start = clip_denoised, cond=cond)
    ...
    def p_losses(self, x_start, t, noise = None, cond=None):
        s0 = time.time()
        b, c, n = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))   #(b,1,3)
    ...
    def forward(self, img, cond, *args, **kwargs):
        ...
        img = normalize_to_neg_one_to_one(img)  #做到了  (b,1,3)
        loss, output = self.p_losses(img, t, cond=cond,  *args, **kwargs)
        pre_label = unnormalize_to_zero_to_one(output)
...
class ST_Diffusion(nn.Module):
    def __init__(self, num_units=20, self_condition=False, condition=True, cond_dim=64, output_dim=17):
        ...
        all_dim = time_dim + cond_dim
        self.linears_mlp = nn.Sequential(
    ...
    def forward(self, x, t, cond=None):
        ...
        pred = self.linears_mlp(cond_all)
